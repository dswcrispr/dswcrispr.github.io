...
$x_{1}$, $x_{2}$ ... 와 같은 확률변수가 $\textit{iid}$ 이고, $x_{i}$의 평균이 유한하다고 가정할 때, 대수의 법칙을 한문장으로 표현하면 $$\textbf{표본의 평균(sample mean)은 모집단 평균(population mean)에 확률적으로 수렴한다.}$$이다.
