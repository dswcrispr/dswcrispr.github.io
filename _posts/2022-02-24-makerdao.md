...
**(1) Collateral Assets**
<br>
Maker protocol은 Ethereum 블록체인 기반에서 작동하는 시스템이므로 Dai를 발행할 수 있는 담보자산으로 주로 Ethereum 기반 자산을 허용하고 있다. 2019년 11월 이전까지는 ETH만을 담보자산으로 허용하였고, Dai를 SCD(Single-collateral Dai), Sai로 부르기도 하였다. 이후  현재 ETH, WBTC, UNI 등이 Maker protocol의 담보자산으로 활용되고 있다. MKR 보유자들은 특정 암호자산의 담보자산 활용 여부 뿐만 아니라 각각 담보자산에 대한 청산비율(liquidation ratio) 등과 같은 risk parmeters 들을 결정한다. 
