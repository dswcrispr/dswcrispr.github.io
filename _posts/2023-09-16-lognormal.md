...
국내 경제학 대학원 박사과정 입시를 준비하는 친구가 입학고사의 기출 문제를 보여줬는데, 풀이와 함께 **log-normal 분포의 성질** 등을 정리해보는 것이 의미가 있을 것이라 생각되어 포스팅으로 남겨본다.
...
위 식에 log를 취하고 충분히 작은 r에 대해 $log(1+r) \approx r$임을 이용하면 아래의 식을 얻는다
...
만약 주식 수익률 $r_t$가 **평균이 0인 정규분포를 따른다면 위 식은 포스팅 서두에 등장한 모 대학 경제대학원 기출문제와 동일한 구조**를 가짐을 알 수 있다. 이를 통해 **T시점의 주식가격의 로그값은 아래와 같이 최초 주식가격의 로그값에 해당 시점까지의 수익률을 누적해서 더한 값으로 근사**해서 표현할 수 있다.  
